...
def fix(x):
    """Canonical NSE symbol, upper-cased. The Pine KEY is derived from this by
    pine_keys() — they are not the same string."""
    x = (x or "").strip().upper()
    if _canon:
        try:
...
def pine_keys(sym):
    """Every form syminfo.ticker might report for this symbol.

    Pine has no '&' in a ticker: TradingView shows Mahindra as M_M, not M&M. The old
    generator emitted the canonical NSE form and the lookup silently missed every
    ampersand symbol. Emitting BOTH costs a handful of extra entries and removes the
    guess about which form TradingView uses for hyphens (NAM-INDIA vs NAM_INDIA).
    """
    out = {sym}
    if "&" in sym:
        out.add(sym.replace("&", "_"))
    if "-" in sym:
        out.add(sym.replace("-", "_"))
    return sorted(out)
...
pairs = sorted({(pk, v[0][1]) for k, v in best.items() for pk in pine_keys(k)})
...
{emit("_b", chunks(codes))}
    ...
    _out
'''
# Replace BETWEEN MARKERS. The old code truncated S4Core at '// SECTOR MAP' and
# appended, which was safe only while the map was the last block in the file. It is
# now 76% of the way through, so that would have deleted 16 exports — wallCheck,
# oiNarrative, optText, rrgSecTxt among them. Markers make the operation bounded.
A, B = '// <SECTOR_MAP_START>', '// <SECTOR_MAP_END>'
core = io.open('S4Core.pine', encoding='utf-8').read()
if A not in core or B not in core:
    raise SystemExit('S4Core.pine has no <SECTOR_MAP_START>/<SECTOR_MAP_END> markers')
i, j = core.index(A), core.index(B) + len(B)
head = (A + '  — regenerated by gen_sector_map.py; do not hand-edit.' + chr(10))
io.open('S4Core.pine', 'w', encoding='utf-8', newline=chr(10)).write(
    core[:i] + head + pine.strip(chr(10)) + chr(10) + B + core[j:])
