...
def compute_decision(rec: dict, ctx: dict, cmp_px, mansfield) -> dict:
    ...
    # Daily PA battery (v67-mirror, 11 conditions) — same source as the Step-5 workflow
    # trigger, so both views speak one "trigger" language.
    ...
    g3_checks = [(f"Catalyst firing ({catalyst})", cat_on), ("Not over-extended", fresh and not_ob),
                 ...
                 (f"PA trigger fired{(' · ' + _pa_names) if pa_fired else ''}", pa_fired)]
...
def compute_workflow(rec, ctx, cmp_px, mansfield) -> dict:
    ...
    # PA pattern battery (v67-mirror, 11 conditions) — status in SETUP, TRIGGER in Step 5
    _pa_pats = _g(ctx, "pa_patterns", default=[]) or []
    _pa_tier = sum(t for _, f, t, _ in _pa_pats if f)
    ...
    steps = [
        ...
        dict(n=5, title="TRIGGER", sub="Daily PA battery + intraday confirm", hard=False, ok=None, manual=True,
             metrics=[("PA trigger", (_pa_names if pa_fired else "none yet"), pa_fired),
                      ("Σ tier", (f"+{_pa_tier}" if _pa_tier else "0"), _pa_tier >= 2),
                      ("Confirm on", "75/125m close", None)],
             do_now=((f"TRIGGER LIVE — {_pa_names} (Σ+{_pa_tier}) fired on the daily. "
                      "Confirm on a CLOSED 75/125m bar at the zone → buy-STOP above its high. Never buy the touch.")
                     if pa_fired else
                     ("No daily PA trigger yet. Wait for a closed-bar pattern (VCP-BO / Pocket / 3-Bar / "
                      "Undercut / IB-NR7 …) at the zone, then confirm on 75/125m: buy-STOP above the trigger bar high."))),
...
def _detect_pa_patterns(df: pd.DataFrame, stage: str = "") -> list:
    ...
    try:
        ...
        # VWMA of VOLUME (dry-up), matching pa_field_validator.py + Dashboard v67
        # Pine (ta.vwma(volume,5)). FIX 8-Jul-2026: was (c*v) = vwma of PRICE, which
        # made `vwma5 < vol50` (price < volume avg) trivially True → dry-up leg was a
        # no-op and VCP-BO over-fired on expanding volume. Now a real contraction gate.
