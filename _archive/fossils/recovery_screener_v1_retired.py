#!/usr/bin/env python3
"""
recovery_screener.py :  Commander Recovery Screener (Python Edition) v1.4
...
⚠️  RETIRED 2026-05-20 ⚠️
─────────────────────────────────────────────────────────────────────────
This screener has been marked RETIRED after Nifty 500 backtest validation.
12-anchor (May 2025 – Apr 2026) results:
  • 53 picks total, -5.25% cumulative alpha vs benchmark
  • Worst anchor -21.24% with -28.4% drawdown (single pick Dec 2025)
  • Avg max DD -12.3% (nearly 2× the bull screener)
  • Hit rate 50.0% — essentially coin-flip
  • R/R ratio 0.22 — not viable as standalone strategy

DO NOT use for live trading. Module is preserved for:
  - Historical analysis and post-mortem review
  - Backtest framework compatibility (validation.py still accepts screener='recovery')
  - Potential future redesign on a different premise (mean-reversion on
    oversold quality names rather than turnaround catalysts)

Use bull_screener.py for live selection — it has validated +14.81% cumulative
alpha over the same backtest period.
─────────────────────────────────────────────────────────────────────────
...
CONFIG = {
    ...
    "cb_drawdown_pct"          : 25.0,  # Pillar 1: ≥N% drawdown from 60D high (v1.3: 15 → 25, aligns with Capitulation Screener v1.3)
    ...
    # -- Fundamentals (RFF) ---------------------------------------------------
    "rff_min_score"            : 1,     # minimum RFF score to pass (0 = disabled)
