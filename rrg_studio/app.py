    ...
    with c_cfg1:
        calc_mode_choice = st.selectbox(
            ...
            help=(
                ...
                "axes get separate lookbacks (one shared N cannot serve both), then an affine "
                "rescale fitted to 17 names from 19-May-2026. Ratio error 8.65 → 2.12, "
                "momentum 2.22 → 0.81. COST: quadrant agreement drops 94% → 88%, because the "
                "rescale pushes borderline names across the 100 line. If you care about the "
                "QUADRANT more than the number, use the 39-Wk preset. Lookback/Smoothing are "
                "ignored by CALIBRATED — it carries its own per-axis values.\n\n"
