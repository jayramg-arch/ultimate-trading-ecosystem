...
# ── THEME JS — inline style injection beats emotion cache ─────────────
import streamlit.components.v1 as _components
_components.html("""
<script>
(function applyButtonStyles() {
    function styleButtons() {
        // All buttons in main area
        document.querySelectorAll('section.main button, [data-testid="stMainBlockContainer"] button').forEach(function(btn) {
            var testid = btn.getAttribute('data-testid') || '';
            if (testid === 'baseButton-primary') {
                btn.style.setProperty('background', '#1a4d2e', 'important');
                btn.style.setProperty('background-color', '#1a4d2e', 'important');
                btn.style.setProperty('border', '1px solid #238636', 'important');
                ...
                btn.style.setProperty('background', '#0d1b2a', 'important');
                btn.style.setProperty('background-color', '#0d1b2a', 'important');
                btn.style.setProperty('border', '1px solid #2d4060', 'important');
        ...
        document.querySelectorAll('[data-testid="stSidebar"] button').forEach(function(btn) {
            ...
            if (testid === 'baseButton-primary') {
                ...
                btn.style.setProperty('background', '#1a4d2e', 'important');
                btn.style.setProperty('background-color', '#1a4d2e', 'important');
                btn.style.setProperty('border', '1px solid #3fb950', 'important');
                ...
                btn.style.setProperty('border', '1px solid #2d4060', 'important');
                btn.style.setProperty('border-radius', '4px', 'important');
                btn.style.setProperty('color', '#adbac7', 'important');
                btn.style.setProperty('font-size', '0.78rem', 'important');
                btn.style.setProperty('width', '100%', 'important');
                btn.style.setProperty('text-align', 'left', 'important');
                btn.querySelectorAll('*').forEach(function(el) {
                    el.style.setProperty('color', '#adbac7', 'important');
                });
            }
        });
    }
    ...
    // Run immediately
    styleButtons();
    // Run again after delays to catch dynamically rendered buttons
    setTimeout(styleButtons, 300);
    setTimeout(styleButtons, 800);
    setTimeout(styleButtons, 1500);

    // MutationObserver — re-applies whenever DOM changes (page switch, rerun)
    var observer = new MutationObserver(function(mutations) {
        var hasButtonChange = mutations.some(function(m) {
            return Array.from(m.addedNodes).some(function(n) {
                return n.nodeType === 1 && (n.tagName === 'BUTTON' || n.querySelector && n.querySelector('button'));
            });
        });
        if (hasButtonChange) {
