...
st.set_page_config(page_title="Trade Journal v6", layout="wide", page_icon="📘", initial_sidebar_state="expanded")
...
try:
    if os.path.exists("portfolio.csv"):
        ...
        for _, r in p_df.iterrows():
            # Normalize Ticker: "NSE:RELIANCE" -> "RELIANCE"
            t_raw = str(r.get('Ticker', '')).strip().upper()
            t_norm = t_raw.replace("NSE:", "").replace("BSE:", "")
            if t_norm:
                portfolio_data[t_norm] = {
...
for sym in active_symbols:
    ...
    lookup_sym = str(sym).strip().upper().replace("NSE:", "").replace("BSE:", "")
...
for opt in nav_options:
    btn_type = "primary" if st.session_state.journal_page == opt else "secondary"
    if st.sidebar.button(opt, key=f"journal_nav_{opt}", width="stretch", type=btn_type):
    ...
    with st.spinner("Forcing Master Portfolio Sync..."):
        ...
        try:
            if os.path.exists("portfolio.csv"):
                ...
                p_df.columns = p_df.columns.str.strip()
                for _, r in p_df.iterrows():
                    # Normalize: "NSE:RELIANCE" -> "RELIANCE"
                    t_clean = str(r.get('Ticker', '')).strip().upper().replace("NSE:", "").replace("BSE:", "")
                    if t_clean:
        ...
        upd_cnt = 0
        for sym in active_symbols:
            s_clean = str(sym).strip().upper().replace("NSE:", "").replace("BSE:", "")
...
.stApp { background: #0b1622; font-family: 'Inter', sans-serif; color: #c9d1d9; }
...
[data-testid='stSidebarContent'] { padding: 0 0 12px 0 !important; overflow-y: auto !important; overflow-x: hidden !important; scrollbar-width: thin; scrollbar-color: #1e3a5f #0a1320; }
...
[data-testid='stSidebar'] h2, [data-testid='stSidebar'] h3 { font-size: 0.60rem !important; font-weight: 600 !important; color: #58a6ff !important; letter-spacing: 1.8px !important; text-transform: uppercase !important; padding: 8px 14px 2px !important; margin: 0 !important; }
...
[data-testid='stSidebar'] [data-testid='metric-container'] label { font-size: 0.58rem !important; font-weight: 400 !important; color: #5a8a9f !important; letter-spacing: 0.8px !important; text-transform: uppercase !important; }
[data-testid='stSidebar'] [data-testid='stMetricValue'] { font-family: 'JetBrains Mono', monospace !important; font-size: 0.82rem !important; font-weight: 600 !important; color: #e3b341 !important; line-height: 1.3 !important; }
[data-testid='stSidebar'] [data-testid='stMetricValue'] > div { font-size: 0.82rem !important; font-weight: 600 !important; color: #e3b341 !important; }
[data-testid='stSidebar'] [data-testid='stMetricDelta'] { font-size: 0.58rem !important; }
[data-testid='stSidebar'] [data-testid='stMetricDelta'] svg { width: 10px !important; height: 10px !important; }
[data-testid='stSidebar'] .stButton > button { background: #0d1b2a !important; border: 1px solid #2d4060 !important; border-radius: 4px !important; color: #adbac7 !important; font-family: 'Inter', sans-serif !important; font-size: 0.78rem !important; font-weight: 400 !important; padding: 5px 10px !important; width: 100% !important; text-align: left !important; min-height: 0 !important; line-height: 1.3 !important; margin: 2px 0 !important; transition: all .1s ease !important; }
[data-testid='stSidebar'] .stButton > button:hover { background: #12243a !important; border-color: #58a6ff !important; color: #e6edf3 !important; }
[data-testid='stSidebar'] .stButton > button[kind='primary'] { background: rgba(35,134,54,0.18) !important; border-color: #238636 !important; color: #3fb950 !important; font-weight: 600 !important; }
[data-testid='stSidebar'] .stButton > button[kind='primary']:hover { background: rgba(35,134,54,0.30) !important; color: #e6edf3 !important; }
[data-testid='stSidebar'] .stButton > button p { font-size: 0.78rem !important; color: inherit !important; }
...
.stApp .stButton > button { background: #0d1b2a !important; border: 1px solid #2d4060 !important; border-radius: 5px !important; color: #c9d1d9 !important; font-family: 'Inter', sans-serif !important; font-size: 0.80rem !important; font-weight: 400 !important; padding: 7px 14px !important; transition: all .15s ease !important; }
.stApp .stButton > button:hover { background: #12243a !important; border-color: #58a6ff !important; color: #e6edf3 !important; box-shadow: 0 0 0 1px rgba(88,166,255,.2) !important; }
.stApp .stButton > button[kind='primary'] { background: rgba(35,134,54,0.15) !important; border-color: #238636 !important; color: #3fb950 !important; font-weight: 500 !important; }
.stApp .stButton > button[kind='primary']:hover { background: rgba(35,134,54,0.28) !important; border-color: #3fb950 !important; color: #e6edf3 !important; }
.stApp .stButton > button p { color: inherit !important; font-size: 0.80rem !important; }
.stApp [data-testid='metric-container'] { background: #0d1b2a !important; border: 1px solid #1e3a5f !important; border-radius: 6px !important; padding: 10px 14px !important; }
.stApp [data-testid='metric-container'] label { font-size: 0.60rem !important; color: #5a8a9f !important; letter-spacing: 1px !important; text-transform: uppercase !important; font-weight: 400 !important; }
.stApp [data-testid='metric-container'] [data-testid='stMetricValue'] { font-family: 'JetBrains Mono', monospace !important; font-size: 1.05rem !important; font-weight: 500 !important; color: #e3b341 !important; }
.stApp [data-testid='metric-container'] [data-testid='stMetricValue'] > div { font-size: 1.05rem !important; color: #e3b341 !important; }
...
.stApp [data-testid='stExpander'] summary:hover { color: #e6edf3 !important; }
...
[data-testid='stTextInput'] input, [data-testid='stNumberInput'] input { background: #080f1a !important; border: 1px solid #1e3a5f !important; border-radius: 4px !important; color: #e6edf3 !important; font-family: 'JetBrains Mono', monospace !important; font-size: 0.78rem !important; }
[data-testid='stSelectbox'] > div > div { background: #080f1a !important; border-color: #1e3a5f !important; color: #e6edf3 !important; border-radius: 4px !important; font-size: 0.78rem !important; }
[data-testid='stChatInput'] textarea { background: #080f1a !important; border: 1px solid #1e3a5f !important; border-radius: 5px !important; color: #e6edf3 !important; font-size: 0.80rem !important; }
...
[data-testid='stToast'] { background: #0d1b2a !important; border: 1px solid #238636 !important; border-radius: 6px !important; color: #c9d1d9 !important; }
[data-testid='stFileUploader'] { background: #0d1b2a !important; border: 1px dashed #1e3a5f !important; border-radius: 6px !important; }
...
if page == "Ledger (Active)":
    ...
    with st.container(border=True):
        col_icon, col_brief = st.columns([0.1, 0.9])
        col_icon.markdown("📡")
        brief = generate_breadth_brief()
        col_brief.markdown(f"**Commander's Breadth Brief:** {brief}")
