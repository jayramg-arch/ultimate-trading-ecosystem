...
import pandas as pd
# dhanhq 2.0.x dropped the top-level `DhanContext` export (only the older SDK had
# it). A HARD import here crashed EVERY consumer of this module at import time —
# including dhan_tv_webhook.py, which only needs check_margin. Guard it so the
# module loads on 2.0.x; DhanContext is used solely by get_client() below, which
# raises a clear error if actually called without it (no webhook path calls it).
from dhanhq import dhanhq
try:
    from dhanhq import DhanContext
except Exception:  # pragma: no cover — dhanhq 2.0.x
    DhanContext = None
...
def get_client(config_path: str | None = None):
    ...
    if DhanContext is None:
        raise RuntimeError(
            "dhan_helpers.get_client() needs DhanContext (older dhanhq SDK), but the "
            "installed dhanhq (2.0.x) does not export it. Use dhan_auth.get_dhan_client() instead."
        )
