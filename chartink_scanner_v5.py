...
# ==========================================
print("\n" + "="*60)
print("🔍 CHARTINK ATLAS SCANNER (V5.0 - FAILSAFE)")
print("="*60)
...
try:
    ...
    # --- THE FIX: Try Default -> Fallback to Timestamp ---
    default_filename = "chartink_results.csv"
    timestamp_filename = f"chartink_results_{datetime.now().strftime('%H%M%S')}.csv"
    ...
    try:
        ...
        print(f"\n⚠️ WARNING: '{default_filename}' is LOCKED.")
        print(f"   (Another program is holding it).")
        print(f"🚀 SWITCHING PLAN: Saving to '{timestamp_filename}' instead.")
        
        df.to_csv(timestamp_filename, index=False)
