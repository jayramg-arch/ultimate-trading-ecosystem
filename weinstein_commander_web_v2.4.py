...
.section-hdr {{
    ...
    letter-spacing: 3px; text-transform: uppercase; margin: 18px 0 10px 0;
    display: flex; align-items: center; gap: 10px;
}}
.section-hdr::after {{ content:''; flex:1; height:1px; background:#1e3a5f; }}

/* ── SUB SECTION LABEL — FIXED: larger, bright, visible ── */
.section-sub-lbl {{
...
/* ── SIDEBAR RADIO (sub-menus) — FIXED: larger, bright text ── */
...
.sb-section-lbl {{
    ...
    font-size: 0.60rem; color: #3d5a6e;
    letter-spacing: 3px; text-transform: uppercase;
    padding: 14px 16px 5px; border-top: 1px solid #1e3a5f;
    margin-top: 2px;
}}
...
with st.sidebar:
    st.markdown("""
    <div style="padding:18px 16px 14px;border-bottom:1px solid #1e3a5f;background:#080f1a;">
      ...
      <div style="font-family:'JetBrains Mono',monospace;font-size:0.55rem;color:#3d5a6e;letter-spacing:3px;margin-top:3px;">COMMANDER WEB v11.0</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="padding:10px 16px 18px;font-family:'JetBrains Mono',monospace;">
      <div style="font-size:0.58rem;color:#3d5a6e;letter-spacing:2px;text-transform:uppercase;margin-bottom:3px;">API Health</div>
      <div style="font-size:0.80rem;font-weight:600;color:{s_color};letter-spacing:0.5px;margin-bottom:10px;">{sys_status}</div>
...
<div class="statusbar">
  ...
  <div class="sb-cell"><div class="sb-label">Total Deployed</div><div class="sb-value" style="color:#e3b341;">₹{total_deployed_g:,.0f}</div></div>
</div>
""", unsafe_allow_html=True)
