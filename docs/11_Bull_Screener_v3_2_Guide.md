# Commander Bull Screener v3.2 — User & Trading Guide
...
- **POS-ACCUM Catalyst Disabled:** Extensive backtesting (May 2025-Apr 2026 across Nifty 500) revealed the POS-ACCUM catalyst produced a -10.04% mean alpha with 0 wins. It has been deactivated to stop bleeding capital.
...
| `LEVELS` | Price · SL with % distance · T1 (2R, ⚠RESIST if blocked) · T2 (3R) · Today's signal (BREAKOUT/PULLBACK/—) |
...
### Inherited from v2.7–v2.9 (still active)

- **v2.7 Hunter sync.** `hunter_weekly_rsi_min` (default 60) and `hunter_daily_adx_min` (default 25). POS-BO requires `wRsiVal ≥ 60 AND ADX ≥ 25`.
...
- An **Alpha Score** (0–100) for setup quality
- A **Confluence Score** (0–6) counting how many catalyst gates are simultaneously active
...
| 6 | **GAP-GO** | Gap-and-Go (≥4% gap, ≥3× volume, intraday range ≥60%) | Swing |
...
| **RS Slope Sensitivity** | `0.2` | Minimum ROC on ×100 scale to register as Rising/Declining. |
...
| **Show Alpha Score Gauge** | `true` | Renders a visual 0–100 gauge for the Alpha Score |
...
### v2.7–v2.9 Inputs (NEW — Backtest-Locked)
...
|---|---|---|---|
| **Hunter Weekly RSI Min** | `60` | 40–80 | v1 FINAL (was 55 pre-tune). POS-BO requires `wRsiVal >= this`. Mirrors `chartink_replay.SCAN_PARAMS["hunter"]["weekly_rsi_min"]`. |
| **Hunter Daily ADX Min** | `25` | 10–50 | v1 FINAL (was 20 pre-tune). POS-BO requires numeric ADX(14) ≥ this. Mirrors `chartink_replay.SCAN_PARAMS["hunter"]["daily_adx_min"]`. |

#### v2 FINAL — Catalyst Filters (Locked 2026-05-10)
| Input | Default | Range | Why |
|---|---|---|---|
| **POS-ACCUM Daily RSI Max** | `50` | 30–70 | v2 LOCKED. POS-ACCUM is suppressed when daily RSI > this. Mirrors `v2_fixes.V2_PARAMS["pos_accum_rsi_threshold"]`. Reason: late-stage POS-ACCUM at high RSI is a chase trap (HCLTECH-style failure documented in `BACKTEST_RESULTS_v2.docx` §6). |
...
| Green bar + volume > 1.5× avg | +20 |
| Green bar + volume > avg (but < 1.5×) | +10 |
| Distance to EMA20 < 5% | +20 |
| Distance to EMA20 5–10% | +10 |
| Macro Edge active (VWMA50 > SMA50) | +10 |
| Macro Edge OFF AND volume < avg | −20 |

**Threshold:** Alpha ≥ 60 required for any active signal. Alpha ≥ 80 = STRONG BUY eligible.
...
| Mansfield RS ≥ 10 | +10 | (additional, on top of above) |
| Mansfield 4w momentum > 0 | +5 | |
| RRG quadrant LEADING | +5 | |
| RRG quadrant WEAKENING | −10 | |
| RRG quadrant LAGGING | −5 | |
| Volume tier — relVol ≥ 2.0 | +15 | |
| Volume tier — relVol ≥ 1.5 | +10 | |
| Volume tier — relVol ≥ 1.0 | +5 | |
| Sector strength (Stage 2 sector) | +5 | approximation via `secStageNum` |
| Sector strength (Stage 4 sector) | −5 | approximation via `secStageNum` |
| Trend template pass | +10 | |
| Distance off 52W high — ≤ 5% | +10 | |
| Distance off 52W high — ≤ 10% | +7 | |
| Distance off 52W high — ≤ 15% | +5 | |
| Distance off 52W high — ≤ 25% | +3 | |
| **Defensive penalty** (off by default) | −10 | If `days_since_pivot_penalty_on=true` AND bars since last `isBreakout` > `days_since_pivot_max` |
...
| 6 | vol_acc_ok: ≥6/20 bars = green + upper 60% + vol > avg | Institutional accumulation evidence |
| 7 | stage2_fresh_ok: wStageWks ≤ 26 | Fresh Stage 2 only — not late-cycle |
...
AND institutional_vol: volume > 2× avg on an OBV-positive bar
AND price_lag: close < highest_high_63_bars × 0.95  (price hasn't run yet)
AND alpha_ok: score ≥ 60
...
AND anti_algo_ok: bar range < 2× ATR (not a fakeout extension bar)
AND volume ≥ 1.5× avg
AND alpha_ok
```
...
### GAP-GO (Gap-and-Go)
```
gap_size: (open - close[1]) / close[1] × 100 ≥ 4%
AND gap_vol: volume ≥ 3× avg50  (institutional gap)
AND intraday_strength: close ≥ low + (high - low) × 0.60
AND alpha_ok
...
| **ATR%** | Current ATR/Close × 100 (volatility %) |
| **STOP SL** | Calculated stop-loss price |
| **T1** | First target at configured R-multiple |
| **T2** | Second target |
| **R:R** | Risk-reward to T1 |
...
> | 3 — Sector RS | `Sector RS Value` + `RRG Quad` | cols 20–21 |
...
> | 7 — stage2_fresh_ok | `Stage Duration (Weeks)` ≤ 26 | col 31 |
> | 8 — ma_sqz_ok | `VCP Tight` (partial) | col 30 |
> | 9 — bb_sqz_ok | `VCP Tight` (partial) | col 30 |
>
> For CSV-only screening, use `Confluence` (col 6) as the POS-BO gate proxy: a `Catalyst=2` stock with `Confluence ≥ 5` has nearly all 9 gates passing.

---

## 8. The SMC Liquidity Sweep Catalyst (+20 Alpha Points)

New in v2.6: when a **Bullish Liquidity Sweep** is detected (price wicks below 5-day low, then closes above it AND above the 50 DMA, with volume > 1.5× avg), the Alpha Score receives a bonus +20 points. This integrates SMC Zones v1.0 logic directly into the screener's quality scoring.
...
|---|---|---|---|---|
| **Growth Leader** | 2 (≤26 wks) | POS-BO | LEADING | ≥80 |
| **Momentum Breakout** | 2 | SWG-BO or SWG-PB | ≥0 | ≥60 |
| **Recovery Play** | 1 or early-2 | Any | ≥−0.5 | ≥40 |
| **Value Base** | 1 | POS-AC | ≥−1 | ≥40 |
| **Speculative** | Any | SWG-REV or GAP-GO | Any | ≥60 |
...
### Pre-Market
1. Scan your watchlist via TradingView's Screener (filter: `catalyst_id > 0` AND `alpha_score ≥ 60`)
2. Sort by Alpha Score descending → top 10 candidates
3. For each: verify on Dashboard that Stage 2, RS = LEADING, VOL SHELF = ✓
4. Check the 9-gate panel for POS-BO candidates — aim for ≥7/9
...
| Hunter (POS-BO) gate | structural only | structural only | **+ wRSI ≥ 60 + numeric ADX ≥ 25** | unchanged | unchanged |
| `f_calc_adx_bool` | bool only | bool only | **renamed `f_calc_adx`, returns [bool, float]** | unchanged | unchanged |
| POS-ACCUM gate | OBV+VCP+weinstein | unchanged | unchanged | **+ dailyRsi ≤ 50 (v2 LOCK)** | unchanged |
...
**Backtest evidence:** v2.7 + v2.8 promotion verdict from 12-anchor walk-forward × 2 universes (raw Nifty500 + filtered Chartink+conviction). Only `pos_accum_rsi_nullout` cleared the cross-universe verification rule (raw +0.14pp α / filtered +0.26pp α, hit-rate held both). Full evidence in `BACKTEST_RESULTS_v2.docx`.
...
| 3 | `Alpha Score` | 0–100 | Setup quality score | **Mandatory gate ≥60**. ≥80 = STRONG BUY eligible. Never enter <60. |
| 4 | `Dashboard Quality Score (0-100)` | 0–100 | Composite from Dashboard v67.0 (RS, vol, trend, breadth) | Tie-breaker when two stocks have equal Alpha. Higher Dashboard score = healthier macro context. |
| 5 | `Alpha Stars (0-5 ★)` | 0–5 | `floor(Alpha/20)` | Quick visual rank. 4★+ = primary watchlist. |
| 6 | `Confluence (0-6 Stars)` | 0–6 | Number of catalyst gates simultaneously firing | ≥3 = unusually strong setup. **But ≥3 alone is not entry** — confirm Stage and RS first (see §13 mistake #4). |
| 7 | `Stage` | float | `1`=Base, `2.1`=Stage 2 Up, `3`=Top, `4`=Downtrend | **Only trade `1` or `2.1`**. Skip every row with `3` or `4`. |
| 8 | `Persona` | 0–5 | `5`=Growth Leader, `4`=Momentum, `3`=Turnaround, `2`=Volatile, `1`=Extended, `0`=Lagging | Match persona to your style. Positional traders want `5/4`. Avoid `0` regardless of Alpha. |
| 9 | `Style` | 0–3 | `3`=Both, `2`=Swing, `1`=Positional, `0`=Wait | Filters out setups that don't match your holding period. |
| 10 | `Catalyst` | 0–6 | `1`=POS-AC, `2`=POS-BO, `3`=SWG-PB, `4`=SWG-BO, `5`=SWG-REV, `6`=GAP-GO, `0`=none | Tells you which playbook section (§6) to follow for this row. `0` = no live signal — use as watchlist only. |
...
| 12 | `Distance to SL (%)` | % | (Price − SL) / Price × 100 | Risk per share. Reject anything > your max-risk% (typically 7–10%). |
...
| 19 | `RS Value (vs Nifty 500)` | float (×100) | Mansfield RS — positive = leading the index, negative = lagging | **Filter ≥ 0** for any entry. ≥ 10 = LEADING. ≤ −5 = LAGGING (skip). |
| 20 | `Sector RS Value` | float (×100) | Stock's RS vs auto-detected sector index | Catches stocks leading the broader market but lagging their sector (rotation risk). Want both row 19 and row 20 ≥ 0. |
| 21 | `RRG Quad` | 1–4 | `1`=Leading, `2`=Weakening, `3`=Lagging, `4`=Improving | **Best entries**: Quad `1` or `4` (Leading or Improving). Avoid `2`/`3` for new positions. |
...
| 23 | `Daily Trend` | ±1 | `1`=daily uptrend, `−1`=daily downtrend | Don't fight the daily. `−1` + Catalyst > 0 = wait for daily to flip. |
| 24 | `Weekly Trend` | ±1 | `1`=weekly uptrend, `−1`=weekly downtrend | Hard filter for positional trades. Reject `−1`. |
...
| 26 | `Distance to 20 EMA (%)` | % | (Price − EMA20) / EMA20 × 100 | **<5%** = optimal swing entry zone. **5–10%** = neutral. **>10%** = extended, wait for pullback. |
| 27 | `Relative Volume (x-avg)` | float | Today's volume / 20-day avg | ≥1.25× = healthy. ≥1.5× = institutional participation. <0.7× = no commitment, downgrade. |
| 28 | `Anti-Algo SWG-BO Gate` | 0/1 | `1` = bar range < 2× ATR (not a stop-hunt wide bar) | **Required for SWG-BO entries**. If `0`, wait for next bar's confirmation before entering. |
...
| 31 | `Stage Duration (Weeks)` | int | Weeks the stock has been in current stage | **Stage 2 sweet spot: 1–26 weeks**. >26 wks in Stage 2 = late-cycle, downgrade size. >40 wks = avoid new entries. |
...
- **The CSV is a snapshot at scan time, not real-time.** `Current Price` may have moved 1–2% by the time you open the file. Always re-quote on TradingView before placing orders.
...
- **`RS Value` is on the ×100 Mansfield scale**, not raw price ratio. A value of `15` means 15 percentage points outperformance over the lookback, not 15× the index.
...
1. **Acting on any catalyst without Alpha ≥ 60** — The score gate is mandatory. A VCP breakout with score 45 has 3+ quality factors missing — the setup is incomplete.
...
4. **Treating Confluence Score ≥ 3 as automatic entry** — Confluence means multiple signals agree, but they could all be confirming a bad macro setup. Always check STAGE and RS first.
...
| `bull_screener.py` | v1.11 | **Catalyst-aware SL multiplier**: POS=4.0× daily ATR, WYC=3.5×, REV=2.5×, SWG=1.5× (unchanged). Previously all catalysts used 1.5× — a positional trade with a swing-sized SL gets knocked out by routine pullbacks within the first 2 weeks. |
...
|---|---|---|---|
| `POS-BO`, `POS-ACCUM` | **4.0×** | ~6-12% | 4-6 month positional hold needs room for normal volatility |
| `WYC-*` (Wyckoff base) | **3.5×** | ~5-10% | Multi-month base structure |
| `REV-CB`, `REV-RS`, `REV-EARLY` | **2.5×** | ~4-7% | 90-day recovery / mean-reversion horizon |
| `SWG-BO`, `SWG-PB`, `SWG-GAP`, `SWG-REV` | **1.5×** | ~2-4% | Swing — unchanged |
...
- **`T1_pct` and `T2_pct` for POS-* are also wider** because targets are computed as `R × SL_distance` (5R / 10R for POS-*). The Risk:Reward ratio is preserved.
- **Recovery screener now also has a wider SL fallback** (`recovery_screener.py` v1.6: safety floor widened from 1.5× → 2.5× daily ATR for REV-*).

### 15.4 Recommended Workflow Adjustment

For POS-* picks, **manually verify the SL price is structurally sound** before placing the order. The 4× ATR rule is a fallback when no structural anchor is available; in practice, the natural Weinstein SL would be:

- POS-BO: 2-3% below the breakout pivot
- POS-ACCUM: below the accumulation base low (often slightly tighter than the 4×ATR fallback)

If your structural SL is *tighter* than the 4×ATR fallback, use the structural SL (and reduce position size to keep 1% risk). If the structural SL would put SL above current close, fall back to the 4×ATR rule.

### 15.5 What Was Rolled Back

These v1.8 / v3.2 changes are NO LONGER in effect:

- ❌ ~~POS-ACCUM disabled~~ — RESTORED to live trading
- ❌ ~~REV-* removed from recovery screener~~ — RESTORED (Wyckoff added alongside, not as replacement; see `09_Recovery_Screener_v1_7_Guide.md` §11)
- ❌ ~~"Bull screener has no edge" verdict~~ — withdrawn; was based on wrong-horizon measurement

### 15.6 Live Trading vs Backtest Numbers

If you read older backtest reports referencing `+14.81% cumulative alpha` (v1.8 verdict) or `−2.23 Sharpe` (Week-3 buggy run) — both were measurement artifacts and should be disregarded. The current honest baseline on properly-windowed validation (n500 × 18mo, catalyst-aware horizons) is:

- Mean matched alpha per trade: **+0.90%**
- Cumulative alpha (18mo): **+8.84%**
- Bootstrap CI95 on mean alpha: **[−1.66%, +3.63%]** (straddles zero — small sample, not statistical confirmation yet)
