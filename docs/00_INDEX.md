...
| 21 | [Mansfield RS — Auto Sector v1.4](./21_Mansfield_RS_AutoSector_v1_4_Guide.md) | `Mansfield_RS_AutoSector_v1.4.pine` | Pine | **NEW (July 2026):** drop-in Mansfield RS with **auto-detected sector benchmark** (v67 hybrid DB+keyword map — no manual sector edits). Plots Index-RS + Sector-RS lines, a **Stage × RS confluence background** (🟢 S2+RS+ / 🟣 S2+RS− weak-leadership / 🟡 S1 / 🟧 S3 / 🔴 S4), and a one-line **Stage · RRG · Sector** label. Stage (`f_sec_stage_lite`) and RRG (`f_calc_rs_logic`, JdK 12) are **verbatim from v67** — label matches the v67 RRG panel cell. Refresh DB via `sector_manager.py export-pine`. |
