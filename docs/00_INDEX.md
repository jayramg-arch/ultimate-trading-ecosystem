...
| 20 | [Chart Markup v1.5](./20_Chart_Markup_v1_5_Guide.md) | `Commander_Chart_Markup_v1.5.pine` | Pine | **NEW (June 2026):** self-contained, timeframe-agnostic chart-reading **scanner** — Stage + Mansfield RS + auto trendlines/channels/wedges + S/R + gaps + classical patterns (flags, double/triple tops, H&S/Cup candidates) + **Anchored VWAP** (Dashboard v67.4.12 port) + **Fibonacci** (Zigzag v6.2 port). Outputs a **decision-brief panel** (Stage · RS↑↓ · setup · conviction 0–7 · nearest S/R · Entry/Stop/Target · verdict) + **descriptive sentence-notes** at each level. Read-only guides (lines not draggable); hand-draw the tradeable trendline. |
