...
| 22 | [Section 4 — Entry Trigger & Price Memory v9.17](./22_Section4_Entry_Trigger_Guide.md) | `Section4_Entry_Trigger_v7.2.pine` (title **v9.17**, library **S4Core/12**) | Pine | **REWRITTEN 31-Jul-2026 (v2.8 → v7.1).** The **execution layer** — and **PART B is the complete end-to-end GM + S4 workflow**. (A) the PA battery (**17 Bull / 10 Recovery**, `pa_patterns.py` mirror, Σ-tier); (B) **LOCATION** — the Institutional **leg-base-leg zone engine** (RBR/DBR/RBD/DBD on Chart+D+W+M, per-TF leg-in/width/ageing, fresh→tested→violated) **replacing Order Blocks (v3.0)**, plus S/R levels with MTTWR grading and R↔S flip, the three anchored VWAPs, and VP VAL/POC; (C) the **VERDICT ladder** — NO TRADE (Stage 3/4) · Recovery · Blue-sky · **Pullback-to-value (v7.1)** · Clear-to-break · Skip — with the trade **type** deciding which reward gate applies; (D) the **trigger-bar latch (v6.0)** so a plan anchors to the bar that fired. ⛔ Two operational rules: the file sits at the **compiled-token ceiling** (every addition needs a paired removal; cost is per *operation*, not per character), and a **recompile DESTROYS your alerts** — re-create every GO alert after every compile. · **UPDATED 19-Aug-2026:** SUMMARY column (§4d), Gate 5 R display-only (§4e), Strike-RRG paste retired, RRG recalibrated. |
