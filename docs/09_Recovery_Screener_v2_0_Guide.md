# Commander Recovery Screener v2.0 — User & Trading Guide
...
── 🛡 RECOVERY GATES X/10 [PRIME/WATCH/WEAK] ── (header with status pill)
...
- A **Signal Code** (0–4) and **Recovery Score** (0–12) as Pine Screener columns
...
|---|---|
| `0` | No signal — no edge active |
| `1` | Climax detected — watching for Turn Bar (Pre-REV-CB) |
| `2` | **REV-CB active** — Capitulation Bottom Bounce confirmed |
| `3` | **REV-RS active** — RS Survivor Breakout |
| `4` | **REV-EARLY active** — NR7/Inside-3 compression reclaim |

When multiple edges are simultaneously true, the highest-value signal (`4 > 3 > 2`) is reported.
...
|---|---|---|
| **Min Daily Turnover (INR)** | `50,000,000` | Liquidity gate — tickers below this are skipped. 50M = ₹5 Cr daily. Calculated as Close × Volume. Raise to 100–200M for large-cap focus. |
...
| **Min Correction from 52W High (%)** | `10.0` | Stock-correction branch of the regime gate — stock passes if it has corrected ≥ this % from its 52W high. |
...
| Input | Default | Explanation |
|---|---|---|
| **Max Drawdown — 60-bar (%)** | `-25.0` | Pillar 1: 60-bar drawdown ≤ this. Default −25 = 25% off the 60-day high. |
...
| **Min Climax Volume Multiplier (× avg)** | `2.0` | Climax volume threshold (used inside Pillar 3 widest+bearish frame). |
...
| **Volume Confirmation — RS/EARLY Breakouts (× avg)** | `1.5` | Breakout volume minimum for both REV-RS and REV-EARLY. |
...
### Section 1 — SETUP (rows 2–8)
| Row | What It Shows |
|---|---|
| **SIGNAL** | Active edge name (REV-EARLY / REV-RS / REV-CB / WATCH (Climax) / NONE). Colour-coded: yellow=EARLY, green=RS, red=CB, orange=WATCH. |
| **SCORE / GRADE** | Recovery Score 0–12 with grade (≥9=A green, ≥6=B orange, <6=C/D red). |
| **STAGE (Weekly)** | Weinstein weekly stage 1–4. Green if Stage 1 or 2 (recovery-eligible). |
...
| **CORRECTION** | % off 52W High. Green ≥25%, orange ≥10%, red <10%. |
| **MARKET** | Market state narrative from the CNX500 regime gate. |

### Section 2 — RECOVERY GATES (rows 10–19)
Shows **10 gates** as ✓/✗ with a pass count header (`x/10`). Header shows PRIME (≥8), WATCH (≥5), or WEAK (<5).

**G1–G5 are universal across all signals:**

| Gate | What It Tests |
|---|---|
| G1 — Regime OK | 3-way regime gate passes (market corrected / reclaiming / stock corrected) |
| G2 — Liquidity OK | Close × Volume ≥ min daily turnover input |
| G3 — Fundamentals | RFF score ≥ your input threshold; shows `(x/2)` detail |
...
**G6–G10 adapt their labels based on the active signal:**

| Signal | G6 | G7 | G8 | G9 | G10 |
|---|---|---|---|---|---|
| **REV-CB / WATCH** | P1 — 60-bar DD | P2 — Red Bars | P3 — Climax Vol | CB Turn Bar | Climax Window |
| **REV-RS** | Strict Trend Up | Higher-Low | RS Breakout Lvl | Volume ≥1.5× | RS Positive |
| **REV-EARLY** | NR7 / Inside-3 | 20-bar Reclaim | Volume ≥1.5× | Volume ≥1.5× | RS Positive |

### Section 3 — RS & QUALITY (rows 21–26)
| Row | What It Shows |
|---|---|
| **RS N500 (Mansfield)** | Mansfield RS ×100 value + RRG quadrant (LEADING/WEAKENING/LAGGING/IMPROVING) |
| **RS MOMENTUM** | 4-bar ROC of Mansfield (RRG y-axis). Positive = accelerating RS. |
| **REL VOLUME** | Volume / 50D avg. Green ≥2×, orange ≥1.5×, grey <1.5×. |
| **RSI(14)** | 14-period RSI. Highlighted green if <30 (oversold = CB setup zone). |
| **RSI(3)** | 3-period RSI fear gauge. Green <10, orange <20. |
| **60-BAR DRAWDOWN** | % drawdown from the 60-bar high. Green if ≤ your Pillar 1 threshold. |

### Section 4 — TRADE LEVELS (rows 28–32)
| Row | What It Shows |
|---|---|
| **PRICE** | Current close |
| **SL (x.x%)** | Calculated stop-loss with % distance in the label |
| **T1 (x.x% up)** | Target 1 with % upside. CB=EMA20; RS/EARLY=2.5R |
| **T2** | Target 2. CB=200 DMA; RS/EARLY=52W High |
| **R:R TO T1** | Risk:Reward ratio to T1. Green ≥2.0, orange ≥1.0, red <1.0. |

### Section 5 — SIGNAL GUIDE (rows 34–39)
...
## 5. Recovery Score Components (0–12)
...
| Mansfield RS ≥ 10.0 (strong positive) | +1 |
| Correction depth ≥ 25% from 52W High | +2 |
| Correction depth ≥ 15% from 52W High | +1 |
| Market in recovery regime | +1 |
| Weekly stage = 1 or 2 | +1 |
| Edge signal = REV-CB or REV-RS/EARLY triggered | +2 |
| Volume on signal bar ≥ 3× avg (exceptional) | +1 |

**Interpretation:**
- **≥ 9 (Grade A):** Highest priority candidate — immediate pipeline to Risk Allocator
- **6–8 (Grade B):** Strong candidate — verify on Dashboard before entering
- **3–5 (Grade C):** Monitor only — wait for additional confirmation
...
|---|---|---|---|
| 1 | **Signal (4=EARLY \| 3=RS \| 2=CB \| 1=Watch \| 0=None)** | 0=No signal, 1=Climax forming (pre-CB), 2=REV-CB active, 3=REV-RS active, 4=REV-EARLY active | Primary filter: `≥2` for actionable signals. `=1` means watch but do not enter. |
| 2 | **Recovery Score (0-12)** | 0–12 composite quality score | Sort descending after signal filter. Grade A=≥9, B=≥6, C=≥3, D=<3. Only trade A/B grades. |
...
| 7 | **Mansfield RS ×100 vs CNX500** | Positive = outperforming CNX500, negative = underperforming. Scaled ×100 (textbook Mansfield). | REV-RS requires RS>0. Values ≥10 = strong; ≥5 = moderate. CB stocks may have negative RS — acceptable. |
...
| 10 | **60-bar Drawdown (%, neg = below)** | Negative = % below 60-bar high. E.g. −28.5 = down 28.5% | Pillar 1 gate: must be ≤ your input (default −25%). Deeper = more capitulation evidence. |
| 11 | **Correction from 52W High (%)** | 0–100, positive = % off 52W high | Stock-leg of regime gate: ≥10% passes. Also scores the composite: ≥25%=+2, ≥15%=+1. |
| 12 | **Relative Volume (× 50D avg)** | 1.0=average day. >1.5=elevated. >3.0=exceptional | Must be ≥1.5× on REV-RS/EARLY signals. ≥2.0× on REV-CB climax bar. ≥3.0× adds +1 to score. |
| 13 | **RSI(3) — Fear Gauge** | 0–100. Very low values (<10) = extreme fear/capitulation | Use to time CB entries: RSI(3) < 10 = deep fear, ideal for Turn Bar watch. High values = avoid buying. |
| 14 | **RSI(14)** | 0–100. <30=oversold, >70=overbought | Context indicator. CB setups often show RSI(14) 20–35 at the climax low. |
| 15 | **Red Bars in Lookback** | Count of down-close bars in the lookback window (default 10) | Pillar 2a: must be ≥5 of 10 for CB. High count = sustained selling = capitulation quality. |
...
| 20 | **Stop Loss Price** | Calculated SL: CB=climax low − 0.5 ATR; RS/EARLY=10-bar low − 0.2 ATR | Your hard exit. If close < SL, thesis is broken — exit. Never move SL down. |
| 21 | **T1 Target Price** | CB=EMA20; RS/EARLY=entry + 2.5 × risk | First profit target. Exit 50–75% of position here to lock in R. |
| 22 | **T2 Target Price** | CB=200 DMA; RS/EARLY=52W High | Full target. Trail remaining position with EMA20. |
| 23 | **R/R Ratio to T1** | (T1 − Entry) / (Entry − SL). ≥2.0 = minimum threshold | Only enter when R:R ≥ 2.0. Below 2.0 the trade is not worth the risk. |
| 24 | **T1 Upside (%)** | % gain if T1 is reached from entry price | Quick scan to discard low-upside setups. REV-CB typically shows 8–20% to T1. |
| 25 | **Stop Loss Distance (%)** | % from entry to stop loss | Cap at 8–10% per your risk rules. SL% × position weight = portfolio risk per trade. |
...
```
→ Sort by Mansfield RS ×100 descending (strongest RS leaders first).
...
2. Filter to Signal ≥ 2 (or Signal=1 for watchlist building)
...
4. **RFF Lite ≠ Full RFF** — The screener uses TTM financials from `request.financial()` which is one period behind. A Score=2/2 is necessary but not sufficient — verify in the Dashboard.
...
| T1 | EMA20 (exit 50–70% of position here). |
...
| Size | Full size if Mansfield RS ≥10; Half if RS 0–10. |
| Confirm | HH/HL pivot structure must hold; volume ≥1.5× through close. |
| T1 | 2.5R from entry. |
| T2 | 52W High. |
...
| Size | Swing allocation. NR7 bars have tighter stops — use full swing if SL distance ≤5%. |
| Confirm | Volume ≥1.5× + reclaim level must hold as support. |
| T1 | 2.5R; trail with EMA20. |
| T2 | 52W High. |
...
| Feature | v1.3 | v1.5 |
|---|---|---|
| Climax Volume Threshold | `2.5×` avg | `2.0×` avg (more inclusive) |
...
| `recovery_screener.py` | v1.6 | **SL safety-floor widened** from 1.5× → 2.5× daily ATR for REV-*. The 90-day recovery horizon needs more room than a swing-sized stop. |
...
- v1.6 fallback (when structural anchor is above close): **2.5× daily ATR** (was 1.5×).

### 13.4 What Was Rolled Back

❌ ~~"Recovery screener retired"~~ — RESTORED. The v1.4 retirement was based on wrong-horizon measurement and is withdrawn.
